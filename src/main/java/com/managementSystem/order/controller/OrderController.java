...
public class OrderController {
    ...
    public ResponseEntity<CreateOrderResponse> createAdminOrder(@RequestBody CreateOrderRequest request, HttpSession session) {
        Admin loginAdmin = (Admin) session.getAttribute("loginAdmin");
        if (loginAdmin == null) return ResponseEntity.status(HttpStatus.UNAUTHORIZED).build();
        return ResponseEntity.status(HttpStatus.CREATED).body(orderService.createOrder(request, loginAdmin));
    }

    @PostMapping("/customer/orders")
    public ResponseEntity<CreateOrderResponse> createCustomerOrder(@RequestBody CreateOrderRequest request) {
        return ResponseEntity.status(HttpStatus.CREATED).body(orderService.createOrder(request, null));
    }

    @GetMapping("/admin/orders")
    public ResponseEntity<Page<GetOrderResponse>> getOrderList(
